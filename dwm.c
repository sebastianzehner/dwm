	...
	int i, x;
   	unsigned int click;
    	...
    	if (ev->window == selmon->barwin) {
        	if (ev->x < ble - blw) {
            	    /* Tag click handling with hide_vacant_tags patch */
            	    i = x = 0;
            	    unsigned int occ = 0;
                	...
                	if (!(occ & (1 << i) || m->tagset[m->seltags] & (1 << i)))
                    	    continue; // skip vacant/inactive tags
                	...
                	return; // click outside of tags
            	    }
        	} else if (ev->x < ble) {
            	    click = ClkLtSymbol;
        	} else if (ev->x < selmon->ww - wstext) {
            	    click = ClkWinTitle;
        	} else if ((x = selmon->ww - RSPAD - ev->x) > 0 &&
...
{
	if (!arg || !arg->v) {
		/* toggle layout only if both layouts are valid */
		if (selmon->lt[0] && selmon->lt[1]) {
			selmon->sellt ^= 1;
		}
	} else if ((Layout *)arg->v != selmon->lt[selmon->sellt]) {
		selmon->sellt ^= 1;
		selmon->lt[selmon->sellt] =
		    selmon->pertag->ltidxs[selmon->pertag->curtag][selmon->sellt] = (Layout *)arg->v;
	}

	/* fallback safety: if current layout is NULL, default to first layout */
